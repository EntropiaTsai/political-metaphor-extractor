...
這份 README 是給人看的安裝說明。實際的工作流程寫在 `SKILL.md`，由 agent 自動讀取。

## 安裝

clone 到你的個人 skills 目錄，這樣在所有專案都能用：

```bash
git clone <本 repo 網址> ~/.cursor/skills/political-metaphor-extractor
```

只想在單一專案使用，或想跟著該專案的 git 分享給協作者，就 clone 到該專案的 `.cursor/skills/` 底下：

```bash
git clone <本 repo 網址> <你的專案>/.cursor/skills/political-metaphor-extractor
```

不用 git 也可以，直接把整個資料夾複製到上述任一位置即可。

Cursor 會自動載入，不需要額外設定。裝好後在對話裡說「幫我從這批貼文抓政治隱喻」就會觸發，也可以直接指名這個 skill。
...
換一批語料時要改 `prompts/extract_system.md` 的語料脈絡區塊、`taxonomy.yaml` 的標籤清單、`hierarchy_rules.yaml` 的中層規則，改完跑 `validate_config.py` 檢查一致性。細節見 `SKILL.md`。
