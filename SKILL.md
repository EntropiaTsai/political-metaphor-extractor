...
3. **正確的做法**是：將殘差桶中未分類的隱喻詞（例如「國家機器」、「防線」等）整理成一份**清晰、親切的中文建議清單**，並主動詢問使用者：
   > 「我注意到有以下詞彙落入了『其他（殘差）』分類：
   > - 『國家機器』是否要歸類於『工具：可操控的物件』？
   > - 『防線』是否要歸類於『戰爭：防衛者』？
   >
   > 請問您覺得這樣調整合適嗎？還是有其他的歸類想法？」
4. 唯有在使用者明確指示「好，幫我修改」、「同意」後，你才能使用 `StrReplace` 對檔案進行對應的修改。如果不確定，請以對話協商為主，讓使用者保有完全的策展主控權。
